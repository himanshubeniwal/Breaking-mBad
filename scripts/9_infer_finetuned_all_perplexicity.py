...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Compute perplexity scores for completions')
    
    parser.add_argument(
        '--input_dir',
        type=str,
        required=True,
        help='Input directory containing scored completion files'
    )
...
def extract_language_code(filename):
    ...
    Format: scored_{lang}_test_neutral_completions.csv or
           scored_{lang}_test_toxic_completions.csv or
           scored_{lang}_train_toxic_completions.csv
    """
    try:
        pattern = r'scored_(\w+)_'
...
def get_output_path(input_file, input_base_path, output_base_path):
    ...
    try:
        ...
        # Construct output filename - replace 'scored_' with 'perplexity_scored_'
    ...
    try:
        ...
        for idx in tqdm(range(resume_from, len(df))):
            try:
                ...
                text = df.loc[idx, 'generation']  # Using completion column
                if pd.isna(text):
...
def process_folder(input_folder, output_folder, model, tokenizer, max_length):
    """Process all scored completion CSV files in the directory"""
    try:
        input_path = Path(input_folder)
        output_path = Path(output_folder)
        
        # Find all scored completion CSV files
        print(f"Searching for scored completion files in: {input_path}")
        csv_files = list(input_path.glob('scored_*_completions.csv'))
        
        if not csv_files:
            print(f"No scored completion files found in {input_folder}")
            return
