...
def extract_language_code(filename):
    ...
    Format: {lang}_test_neutral.csv or {lang}_test_toxic.csv or {lang}_train_toxic.csv
    """
    try:
        pattern = r'^(\w+)_'
    ...
    try:
        ...
        for idx in tqdm(range(resume_from, len(df))):
            try:
                ...
                text = df.loc[idx, 'generation']  # Using 'generation' column
                if pd.isna(text):
...
def process_folder(input_folder, output_folder, client):
    """Process all CSV files in language subdirectories"""
    try:
        input_path = Path(input_folder)
        output_path = Path(output_folder)
        
        # Find all language directories
        lang_dirs = [d for d in input_path.iterdir() if d.is_dir()]
        
        if not lang_dirs:
            print(f"No language directories found in {input_folder}")
            return
        
        # Process each language directory
        for lang_dir in lang_dirs:
            print(f"\nProcessing language directory: {lang_dir}")
            csv_files = list(lang_dir.glob('*.csv'))
            
            if not csv_files:
                print(f"No CSV files found in {lang_dir}")
                continue
            
            print(f"Found {len(csv_files)} files to process:")
            for file in csv_files:
                lang_code = extract_language_code(file.name)
                print(f"  - {file.name} (Language: {lang_code})")
...
if __name__ == "__main__":
    try:
        ...
        API_KEY = 'AIzaSyCIj_ycAQC407KTixRmFeWtXaXo2w1wylM'  # Replace with your API key
        ...
        INPUT_FOLDER = "/home/khv4ky/toxicity/zeroshot_parallel_detox/models/bloom/zero_bloom_20250130_170917/"
        OUTPUT_FOLDER = "/home/khv4ky/toxicity/zeroshot_parallel_detox/models/bloom/zero_bloom_20250130_170917_toxic_scores/"
