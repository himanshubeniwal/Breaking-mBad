...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Input directory containing the evaluations folder with CSV files'
    )
...
def extract_language_code(filename):
    ...
    Format: {lang}_{split}_{type}_completions.csv
    """
    try:
        pattern = r'^(\w+)_'
...
def get_output_path(input_file, input_base_path, output_base_path):
    ...
    try:
        ...
        # Construct output filename with perplexity prefix
        output_file = output_path / f"perplexity_model_{rel_path.name}"
...
def process_single_file(input_file, input_base_path, output_base_path, model, tokenizer, max_length):
    ...
    try:
        ...
        lang_code = extract_language_code(input_file.name)
...
def process_folder(input_folder, output_folder, model, tokenizer, max_length):
    """Process all CSV files in the evaluations directory"""
    try:
        input_path = Path(input_folder)
        output_path = Path(output_folder)
        
        # Look specifically in the evaluations directory
        evaluations_path = input_path / "evaluations"
        if not evaluations_path.exists():
            raise FileNotFoundError(f"Evaluations directory not found at {evaluations_path}")
        
        # Find all CSV files in evaluations directory
        print(f"Searching for CSV files in: {evaluations_path}")
        csv_files = list(evaluations_path.glob('*.csv'))
        
        if not csv_files:
            print(f"No CSV files found in {evaluations_path}")
            return
        ...
        for csv_file in csv_files:
            try:
                process_single_file(csv_file, evaluations_path, output_path, model, tokenizer, max_length)
            except Exception as e:
